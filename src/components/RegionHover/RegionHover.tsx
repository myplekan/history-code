...
export const RegionHover: React.FC<Props> = ({ x, y, width, region }) => {
  return (
    <>
      ...
      ></rect>
      <path
        transform={`matrix(1 0 0 1 ${region === "Луганська" ? x + 50 : x} ${
          y - 10
        })`}
      ...
      {region === "Автономна" && (
        <text transform={`matrix(1 0 0 1 ${x} ${y - 15})`}>
          Республіка Крим
        </text>
      )}
      {region === "Київ" && (
        <text transform={`matrix(1 0 0 1 ${x} ${y - 15})`}>столиця</text>
      )}
      {region === "Севастополь" && (
        <text transform={`matrix(1 0 0 1 ${x} ${y - 15})`}></text>
      )}
      {region !== "Київ" && region !== "Автономна" && region !== "Севастополь" && (
        <text transform={`matrix(1 0 0 1 ${x} ${y - 15})`}>область</text>
      )}
    </>
  );
};
