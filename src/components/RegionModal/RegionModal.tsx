...
export const RegionModal: React.FC<Props> = ({ regionName, setRegion }) => {
  const regionValues = regions.regions.filter(
    (region) => region.id === regionName
  );
  ...
  return (
    <div className="fixed top-0 left-0 w-full h-full bg-black-opacity-60 flex justify-center items-center">
      <div className="bg-white flex items-center gap-16 py-8 px-20">
        ...
        <div>
          <img
            src={process.env.PUBLIC_URL + `/images/emblems/emblem-${id}.png`}
            alt="Emblem"
          />
        </div>
        ...
        <div className="max-w-[510px]">
          ...
          <div className="flex items-center gap-4 text-2xl mb-8 text-gray-100">
            <img
              ...
              src={process.env.PUBLIC_URL + "/images/svg/big-pattern.svg"}
              alt="Pattern"
            />
            <div className="flex flex-col gap-2">
              <span>
                Обласний центр - <strong>{regionalCenter}</strong>
              </span>
              <span>
                Заснована - <strong>{founded}</strong>
              </span>
              <span>
                Площа - <strong>{area}</strong>
              </span>
              <span>
